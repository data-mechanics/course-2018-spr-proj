...
class StatLibrary(dml.Algorithm):
	...
	writes =['bemullen_crussack_dharmesh_vinwah.libraryNoStu','bemullen_crussack_dharmesh_vinwah.libraryStu','bemullen_crussack_dharmesh_vinwah.correlation'] # new data base
	...
	def execute(trial=False):
		...
		decStu = [x for x in dec if x["ETL_LOAD_DATE"][8:10] in ["01","02","03","04","05","06","07","08","09","10","11","12","13","14","15"]]
		decNoStu = [x for x in dec if x["ETL_LOAD_DATE"][8:10] not in ["01","02","03","04","05","06","07","08","09","10","11","12","13","14","15"]]

		janStu = [x for x in jan if x["ETL_LOAD_DATE"][8:10] not in ["01","02","03","04","05","06","07","08","09","10","11","12","13","14","15"]]
		janNoStu = [x for x in jan if x["ETL_LOAD_DATE"][8:10] in ["01","02","03","04","05","06","07","08","09","10","11","12","13","14","15"]]
		...
		#'bemullen_crussack_dharmesh_vinwah.libraryNoStu',
		...
		jsondata = '{"correlation coefficient for students in session":'+str(stucor)+', "correlation coefficient for students not in session":'+str(noncor)+'}'
	...
	def provenance(doc = prov.model.ProvDocument(),startTime= None,endTime=None):
		...
		get_statlib = doc.activity('log:uuid'+str(uuid.uuid4()), startTime, endTime, 
			{'prov:label': 'Calculate the correlation coefficents of Boston library users during university session and not in session compared to Boston\'s city score'})
		...
		libraries = doc.entity('dat:bemullen_crussack_dharmesh_vinwah#statlib', {prov.model.PROV_LABEL:'Libraries Metrics with correlation coefficents',
