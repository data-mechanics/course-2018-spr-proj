...
class example(dml.Algorithm):
    ...
    def execute(trial = False):
        ...
        url = 'https://data.cambridgema.gov/resource/q83f-7quz.json'
        response = urllib.request.urlopen(url).read().decode("utf-8")
        r = json.loads(response)
        s = json.dumps(r, sort_keys=True, indent=2)

        repo.dropCollection("cwsonn_levyjr.Ctrees")
        repo.createCollection("cwsonn_levyjr.Ctrees")

        repo["cwsonn_levyjr.Ctrees"].insert_many(r)
