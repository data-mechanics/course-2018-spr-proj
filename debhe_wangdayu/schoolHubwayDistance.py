...
class schoolHubwayDistance(dml.Algorithm):
    ...
    reads = ['debhe_wangdayu.allSchool', 'debhe_wangdayu.hubwayStation']
    writes = ['debhe_wangdayu.schoolHubwayDistance']
    ...
    def execute(trial = False):
        ...
        schools = repo['debhe_wangdayu.allSchool'].find()
        ...
        for row_1 in schools_1:
            ...
            for row_2 in bikeStations_1:
                s = (row_1['schoolName'], row_2['station'], (float(row_1['X']) - float(row_2['X']))**2 + ( float(row_1['Y']) - float(row_2['Y']) )**2 )
            ...
            for (k, b, v) in allDistance:
                if(key == k):
                    if(v < minD):
                        station = b
                        minD = v
            minDistance.append({'schoolName': school , 'hubwayStation': station, 'Distance': minD})

        # save the information to the database
        repo.dropCollection("schoolHubwayDistance")
        repo.createCollection("schoolHubwayDistance")

        repo['debhe_wangdayu.schoolHubwayDistance'].insert_many(minDistance)
        repo['debhe_wangdayu.schoolHubwayDistance'].metadata({'complete': True})
        print("Saved schoolHubwayDistance", repo['debhe_wangdayu.schoolHubwayDistance'].metadata())
    ...
    def provenance(doc=prov.model.ProvDocument(), startTime=None, endTime=None):
        ...
        schoolHubwayDistance = doc.entity('dat:debhe_wangdayu#schoolHubwayDistance',
