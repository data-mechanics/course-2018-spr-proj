...
class income_versus_commuting(dml.Algorithm):
    contributor = 'fjansen'
    reads = ['fjansen.householdincome', 'fjansen.commuting']
    writes = ['fjansen.income_versus_commuting']
    ...
    def execute(trial=False):
        ...
        for group in agg_inc_cols:  # group = each list within agg_inc_cols
            for col in group:  # col = each entry within each 'group'
                result[col] = (result[col].str.replace('%',
                                                       ''))  # remove the % sign from all data and convert it to a float so we can perform math ops later
                result[col] = result[col].astype(float)

        new_inc_cols = ['<34,999 %', '35,000-74,999 %',
                        '>=75,000 %']  # names of new columns (corelate to the lists in agg_inc_cols)
        idx = 0
        ...
        repo['fjansen.income_versus_commuting'].insert_one(merged_data)
    ...
    def provenance(doc=prov.model.ProvDocument(), startTime=None, endTime=None):
        ...
        doc.add_namespace('householdincome', 'dat:fjansen#householdincome')
        doc.add_namespace('commuting', 'dat:fjansen#commuting')

        ## Agents
        this_script = doc.agent('alg:fjansen#income_versus_commuting',
        ...
        ## Entitites
        resource1 = doc.entity('dat:fjansen#householdincome', {'prov:label': 'Household Income by Neighborhood',
                                                               prov.model.PROV_TYPE: 'ont:DataResource',
                                                               'ont:Extension': 'json'})
        resource2 = doc.entity('commuting: dat:fjansen#commuting', {'prov:label': 'Means of Commuting by Neighborhood',
                                                                    prov.model.PROV_TYPE: 'ont:DataResource',
                                                                    'ont:Extension': 'json'})
        ...
        commuting = doc.entity('dat:fjansen#commuting', {prov.model.PROV_LABEL: 'Means of Commuting by Neighborhood',
                                                         prov.model.PROV_TYPE: 'ont:DataSet'})
