...
class Listener(StreamListener):
    ...
    with open(two_up + path_to_file + "/countries.json", mode = 'r', encoding = 'utf-8-sig') as f:
    ...
    def on_status(self, status):
        ...
        if not tmp_json["coordinates"] is None:
            ...
            print(Listener.contained_in (point) )
            location_enabled = True
        elif not tmp_json["geo"] is None:
            point = Point(tmp_json["geo"]["coordinates"])
            print(Listener.contained_in (point) )
            location_enabled = True
        elif not tmp_json["place"] is None:
            print(tmp_json["place"]["country"] )
            location_enabled = True
        #else:
    ...
    def getTweetsByGPS(self, stop_at_number, longitude_start, latitude_start, longitude_finish, latitude_finish):
        try:
...
def main():
    ...
    listener.getTweetsByWords(stop_at_number = 10000)
